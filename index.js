...
// APIs
const inspirationAPI_URL = "https://zenquotes.io/api/random";
...
app.get("/", async (req, res) => {
  try {
    const result = await axios.get(inspirationAPI_URL);
    const data = {
      type: "inspiration",
...
app.post("/", async (req, res) => {
  ...
  try {
    switch (
      choice // switch to get proper API
    ) {
      case "inspiration":
        result = await axios.get(inspirationAPI_URL);
        data = {
          type: choice,
          quote: result.data[0].q,
          author: result.data[0].a,
